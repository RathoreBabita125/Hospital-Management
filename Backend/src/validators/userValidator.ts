...
export const validateUserData = (userData: UserDetails, inputField: string[]): Boolean => {
    ...
    if (inputField.includes('password') && inputField.includes('newPassword') && inputField.includes('confirmPassword')) {
    ...
    else if (inputField.includes('password') && inputField.includes('confirmPassword')) {
        if (confirmPassword === "" || confirmPassword?.trim() === "") {
            throw new Error("Confirm password is required.");
        }
